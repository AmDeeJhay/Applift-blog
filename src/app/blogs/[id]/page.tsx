...
import { getPostById, getCommentsByPostId } from "@/lib/api-functions"; // Import API functions
import { blogPosts } from "@/lib/blog-data"; // Import blogPosts from blog-data

// Generate static paths - can still use local data for build time
...
export async function generateMetadata({
  ...
  // Try to get post from API first, fall back to local data
  let post;
  try {
    post = await getPostById(blogId);
  } catch (error) {
    // Fall back to local data if API fails
    post = blogPosts.find((post) => post.id === blogId);
  }
...
export default async function BlogPost({ params }: BlogPageParams) {
  ...
  // Find related posts - could also use API but using local data for simplicity
